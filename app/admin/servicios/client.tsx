...
export default function ServiciosClient({ businessId, initialServices, initialWorkers }: any) {
  ...
  async function addService() {
    ...
    if (error || !data) {
      setFormError(
        error?.message.includes("allows_concurrent")
          ? "Falta aplicar una actualización en la base de datos (migration_03). Avísale a soporte."
          : "No se pudo guardar el servicio: " + (error?.message ?? "error desconocido")
      );
      return;
    }
  ...
  async function saveEdit(id: string) {
    ...
    if (error) {
      setFormError("No se pudo guardar el cambio: " + error.message);
      return;
    }
