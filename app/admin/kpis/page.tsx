...
export default async function KpisPage() {
  ...
  const tasaRechazo = totalDecididas ? Math.round((rejected.length / totalDecididas) * 100) : 0;

  const clientesUnicos = new Set(
    list.map((a: any) => (a.client_email || a.client_phone || a.client_name || "").toLowerCase().trim()).filter(Boolean)
  ).size;
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="eyebrow mt-1 mb-2">Dinero</p>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <Kpi icon="✅" label="Citas completadas" value={completed.length.toString()} />
      </div>

      <p className="eyebrow mt-4 mb-2">Actividad</p>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <Kpi icon="📈" label="Tasa de aceptación" value={`${tasaAceptacion}%`} />
        <Kpi icon="📉" label="Tasa de rechazo" value={`${tasaRechazo}%`} />
      ...
      <p className="eyebrow mt-4 mb-2">Negocio</p>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-10">
        ...
        <Kpi icon="👥" label="Clientes únicos" value={clientesUnicos.toString()} />
      </div>
